...
public class SecKillController implements InitializingBean {
    ...
    @ResponseBody
    public Result<Integer> doSecKill(SecKillUser secKillUser, @RequestParam("goodsId") long goodsId, @PathVariable("path") String path) {
        if (secKillUser == null) {
            return Result.error(CodeMsg.SESSION_ERROR);
        }
        //    验证path
        boolean check = secKillService.check(secKillUser, goodsId, path);
        if (!check) {
            return Result.error(CodeMsg.REQUEST_SECKILL_ERROR);
        }
        //内存标记，减少redis访问
...
//        redisService.delete(GoodsKey.getGoodsList, "");
    ...
    @ResponseBody
    @AccessLimit
    public Result<String> toSecKillPath(HttpServletRequest request, SecKillUser seckillUser, @RequestParam("goodsId") long goodsId, @RequestParam(value = "verifyCode", defaultValue = "0")
            int verifyCode) {
        if (seckillUser == null) {
    ...
    @ResponseBody
    public Result<String> toSecKillVerifyCode(HttpServletResponse response, SecKillUser seckillUser, @RequestParam("goodsId") long goodsId) {
        if (seckillUser == null) {
            return Result.error(CodeMsg.SESSION_ERROR);
        }
        try {
